...
import {
  ...
  KeyboardAvoidingView, Platform, SafeAreaView, Alert, TextInput, ActivityIndicator, Modal,
} from "react-native";
              ...
              {playerNames.map((name, i) => (
                <View key={i} style={styles.nameRow}>
                  <View style={[styles.nameAvatar, { backgroundColor: colors.primaryLight }]}>
                    <Text style={[typography.btn2, { color: colors.isDark ? "#ffffff" : colors.primary }]}>{name.trim() ? name.trim()[0].toUpperCase() : i + 1}</Text>
                  </View>
